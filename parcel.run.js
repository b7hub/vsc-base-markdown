...
const entryFiles = [path.resolve(__dirname, "./index.html")];

const getOptions = ({ publicUrl = "/" }) => {
  // Bundler 选项
  const options = {
    outDir: path.resolve(__dirname, "./dist"), // 将生成的文件放入输出目录下，默认为 dist
...
async function runParcel (options) {
...
async function boost ({ extensionPath }) {
  const options = getOptions({ publicUrl: extensionPath })
  const bundle = await runParcel(options);
